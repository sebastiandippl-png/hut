...
<section class="card">
    ...
    <?php else: ?>
        <p class="form__hint">Drag links by the handle to sort them within each category. The order is saved automatically.</p>
        ...
        <div class="admin-links__categories">
            <?php foreach ($linksByCategory as $group): ?>
                <div class="admin-links__category">
                    <h3 class="admin-links__category-title"><?= htmlspecialchars((string) $group['name']) ?></h3>

                    <?php if (empty($group['items'])): ?>
                        <p class="empty-state">No links in this category.</p>
                    <?php else: ?>
                        <div
                            class="admin-links__list"
                            data-sortable-list
                            data-sort-endpoint="/admin/links/reorder"
                            data-category-id="<?= $group['id'] === null ? '' : (int) $group['id'] ?>"
                        >
                            <?php foreach ($group['items'] as $link): ?>
                                <div class="admin-links__item" draggable="true" data-sortable-item data-sort-id="<?= (int) $link['id'] ?>">
                                    <button class="admin-sort-handle" type="button" aria-label="Drag to reorder" title="Drag to reorder">⋮⋮</button>
...
<section class="card">
    ...
    <?php else: ?>
        <p class="form__hint">Drag categories by the handle to sort them. The order is saved automatically.</p>
        <div class="admin-categories__list" data-sortable-list data-sort-endpoint="/admin/link-categories/reorder">
            <?php foreach ($categories as $category): ?>
                <div class="admin-categories__item" draggable="true" data-sortable-item data-sort-id="<?= (int) $category['id'] ?>">
                    <button class="admin-sort-handle" type="button" aria-label="Drag to reorder" title="Drag to reorder">⋮⋮</button>
